...
// ============================================================
// ORDER SECTION - Tek sipariş gösterimi
// ============================================================
...
function OrderSection({
  ...
  const statusConfig = {
    ...
    confirmed: { label: 'ONAYLANDI', color: 'var(--gold)' },
    preparing: { label: 'HAZIRLANIYOR', color: 'var(--gold)' },
    ready: { label: 'HAZIR', color: 'var(--super)' },
    on_way: { label: 'YOLDA', color: 'var(--super)' },
    delivered: { label: 'TESLİM', color: 'var(--olive)' },
    cancelled: { label: 'İPTAL', color: 'var(--ink-3)' },
  };
  const cfg =
    statusConfig[order.status as keyof typeof statusConfig] || statusConfig.received;
  ...
  return (
    <div className="px-6 py-4">
      ...
      <ul className="space-y-1.5 pl-1">
        {order.items.map((item) => (
          <li
            ...
            style={{
              color: item.status === 'cancelled' ? 'var(--ink-3)' : 'var(--ink)',
          ...
          >
            <span
              ...
              style={{
                fontFamily: 'var(--f-mono)',
                minWidth: 24,
                color: item.is_complimentary ? 'var(--gold)' : 'var(--accent)',
              }}
            >
              {item.quantity}×
            </span>
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-1.5 flex-wrap">
                ...
                {!item.is_complimentary && onGiftItem && (
                  <button
                    onClick={() => onGiftItem(item.id)}
                    className="text-[9px] uppercase px-1.5 py-0.5 rounded transition-all hover:scale-[1.05]"
                    style={{
                      fontFamily: 'var(--f-mono)',
                      fontWeight: 700,
                      letterSpacing: '0.12em',
              ...
              {item.complimentary_reason && item.is_complimentary && (
                <div
      ...
      {order.note && (
        <div
          className="mt-2 px-2.5 py-1.5 rounded-[8px] text-xs italic"
          style={{
      ...
      {!isPaid && (
        <div className="mt-3 flex items-center gap-2 justify-end flex-wrap">
          ...
          {showPayButton && onPay && (
            <button
              onClick={onPay}
              className="h-8 px-3 rounded-[8px] text-xs font-semibold flex items-center gap-1 transition-all hover:opacity-90 active:scale-[0.98]"
              style={{
                background: 'var(--accent)',
                color: '#FAF5EA',
                fontFamily: 'var(--f-mono)',
